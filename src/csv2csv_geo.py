...
with open('./sz/link_geo.csv','r') as fr:
    lines = fr.readlines()
with open('./sz/link_geo_temp1.csv','w') as fw:
    for line in lines:
        fw.write(line.replace('LINESTRING (',''))

# 清洗字符")"
with open('./sz/link_geo_temp1.csv','r') as fr1:
    lines1 = fr1.readlines()
with open('./sz/link_geo_temp2.csv','w') as fw1:
    for line1 in lines1:
        fw1.write(line1.replace(')',''))

# 清洗字符", "
with open('./sz/link_geo_temp2.csv','r') as fr2:
    lines2 = fr2.readlines()
with open('./sz/link_geo_temp3.csv','w') as fw2:
    for line2 in lines2:
        fw2.write(line2.replace(', ',','))

# 根据","对经纬度进行拆分,使得每一行只有一个经纬度
with open('./sz/link_geo_temp3.csv', 'r') as infile, open('./sz/link_geo_temp4.csv', 'w') as outfile:
...
with open('./sz/link_geo_temp4.csv','r') as fr3:
    lines3 = fr3.readlines()
with open('./sz/link_geo_new.csv','w') as fw3:
    fw3.write('lon,lat\n')
    for line3 in lines3:
