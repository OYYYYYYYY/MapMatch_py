...
def main(argv):
    ...
    with open('./data/cd_taxi/link_single_temp.csv','w') as fw:
        fw.write('link_id,osm_way_id,from_node_id,to_node_id,lng,lat\n')
        for s in range(len(lane_id)):
            fw.write((str(int(lane_id[s]))+','+str(osm_id[s])+','+str(int(from_id[s]))+','+str(int(to_id[s]))+','+str(lng[s])+','+str(lat[s])+'\n'))
       
    df = pd.read_csv("./data/cd_taxi/link_single_temp.csv")
    ff = df[['link_id','osm_way_id','from_node_id','to_node_id','lng','lat']]
    df.drop_duplicates(inplace=True)
    df = df.loc[~(df['link_id'].eq(-1))]

    with open(output,'w') as fww:
        fww.write('link_id,osm_way_id,from_node_id,to_node_id,lng,lat\n')
        for line in df.values:
            fww.write((str(line[0])+','+str(line[1])+','+str(line[2])+','+str(line[3])+','+str(line[4])+','+str(line[5])+'\n'))
